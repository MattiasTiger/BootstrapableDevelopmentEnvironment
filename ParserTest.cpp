...
bool handler_character_(StatementTree & st, Pattern & p)
{
	st.statement = Statement("["+p.pattern+"]");
	st.isList = false;
	return true;
...
bool handler_parentesis_(StatementTree & st, Pattern & p)
{
...
void ParserTest::run()
{
	...
	//p.addPattern(Pattern("xyz", handler_character_));
	p.addPattern(Pattern("+$", handler_add));
	p.addPattern(Pattern("($)", handler_parentesis_));
	...
	std::cout << p.patternTrie.toString() << "\n";
